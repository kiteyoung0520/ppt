// 🔒 安全與配置定數 (與前端 src/services/api.js 保持一致)
...
function doPost(e) {
  try {
    ...
    if (payload.securityToken !== SECURITY_HANDSHAKE_TOKEN) {
      return errorResponse("🚨 偵測到未經授權的連線請求！存取已被系統自動阻斷。");
    }
    ...
    if (rowData[1] == password) {
      ...
      if (expiryDate) {
        ...
        if (now  new Date(expiryDate)) {
          return errorResponse(⚠️ 您的會籍已到期 ( + Utilities.formatDate(new Date(expiryDate), GMT+8, yyyy-MM-dd) + )nn +
                               【續約贊助方案】n +
                               💸 贊助 100 元 (含嘉義文教 50 元、嘉義分苑起厝 50 元)n +
                               📩 贊助方式：請洽嘉義分苑或 kiteyoung@gmail.com);
        }
      }
      
       修復：登入時直接抓取並回傳金幣等資料，防止歸零
      var stats = { coins 0, currentPlant '黃花風鈴木', plantStage 0, unlockedPlants [], exp 0, essence {light0, rain0, soil0}, streak 0 };
        ...
        if (f) {
          ...
          if (typeof rawB === 'string' && rawB.startsWith('{')) {
            try {
              ...
              stats.essence = oldJson.essence  {light0, rain0, soil0};
            ...
            try {
              ...
              stats.essence = extra.essence  {light0, rain0, soil0};
...
function handleApply(payload) {
  ...
  var license = LG- + Math.floor(Math.random()9000+1000) + - + Math.floor(Math.random()9000+1000);
   ...
   1. 寫入 Users (AUserID, BPass, CApiKey, DRegDate, EDevices, FExpiry, GEmail)
  if (userSheet) userSheet.appendRow([userId, password, , new Date(), , , email]);
  
   2. 寫入 Licenses (AUserID, BLicenseKey)
  ...
  if (statsSheet) {
    var extraJson = JSON.stringify({ essence {light0, rain0, soil0}, streak 1, lastStudyDate new Date().toDateString() });
  ...
  try {
    ...
    var body = 親愛的  + userId + ：nn您的入園申請已自動通過！n🔑 您的啟動金鑰： + license + nn請回到登入頁面進行激活。;
  ...
  if (sheet.getName().toLowerCase() === applications && range.getColumn() === 5) {
    ...
    if (newValue === Approved) {
      ...
      lSheet.appendRow([userId, license]);
       ...
       BCoins, CPlant, DStage, FUnlocked, GExp, KExtraJson
      var statsSheet = ss.getSheetByName(UserStats);
      if (statsSheet) {
        var extraJson = JSON.stringify({ essence {light0, rain0, soil0}, streak 1, lastStudyDate new Date().toDateString() });
        statsSheet.appendRow([userId, 100, 黃花風鈴木, 0, 0, '[黃花風鈴木]', 0, [], [], {}, extraJson]);
      ...
      try {
        ...
        var body = 親愛的  + userId + ：nn您的語林之境 30 天探索權限即將在 5 天後到期。n +
                   為了維持 AI 育苗室與精華系統的運作，邀請您參與贊助續約：nn +
                   💎 贊助金額：100 元n +
                      (嘉義文教 50 元、嘉義分苑起厝 50 元)n +
                   📩 贊助方式：請洽嘉義分苑 或 聯繫 kiteyoung@gmail.comnn +
...
function handleGetUserStats(payload) {
  ...
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var stats = { coins 0, currentPlant '黃花風鈴木', plantStage 0, unlockedPlants [], exp 0, essence {light0, rain0, soil0}, streak 0 }, savedWords = [];
  ...
  if (wSheet) {
    wSheet.createTextFinder(payload.userId).matchEntireCell(true).findAll().forEach(function(r){
      var v = wSheet.getRange(r.getRow(), 1, 1, 9).getValues()[0];
       AUserID, BWord, CIPA, DDefinition, EContext, FContextChn, GDate, HID, IMetadata
      savedWords.push({ 
        word v[1], 
        pronunciation v[2], 
        meaning v[3], 
...
function handleUpdateUserStats(payload) {
  ...
  if (payload.savedWords) {
    ...
    payload.savedWords.forEach(function(w){
      ...
      var row = [payload.userId, w.word, w.pronunciation, w.meaning, w.exampleSentence, w.exampleTranslation, new Date(w.addedAtDate.now()), id, metaData];
...
function successResponse(d) { return ContentService.createTextOutput(JSON.stringify({status'success', datad})).setMimeType(ContentService.MimeType.JSON); }
function errorResponse(m) { return ContentService.createTextOutput(JSON.stringify({status'error', messagem})).setMimeType(ContentService.MimeType.JSON); }
    ...
    if (expiryDate instanceof Date && email) {
      ...
      if (expiryStr === checkStr) {
        try {
          ...
          var body = 親愛的培育者  + userId + ：nn +
                     您的語林之境會籍即將於  + Utilities.formatDate(expiryDate, GMT+8, yyyy-MM-dd) +  到期。nn +
                     為了讓您的守護靈能持續成長，我們誠摯邀請您參與贊助方案：n +
                     💸 贊助 100 元 (含嘉義文教 50 元、嘉義分苑起厝 50 元)n +
                     📩 贊助方式：請洽嘉義分苑或 kiteyoung@gmail.comnn +
                     感謝您對語林的守護！🌿✨;
